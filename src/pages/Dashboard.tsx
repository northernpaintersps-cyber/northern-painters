  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl">
          ...
          <div>
            ...
            {todayJobs.length === 0
              ...
              : (
                <div className="space-y-2">
                  {todayJobs.map((j: any) => (
                    <div key={j.id} className="flex items-start gap-2 bg-gray-50/60 rounded-lg px-3 py-2">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-900 truncate">{j.client || '—'}</p>
                        <p className="text-xs text-gray-500 truncate">{j.address || ''}</p>
                      </div>
                      <span className={`text-xs px-1.5 py-0.5 rounded font-medium shrink-0 ${
                        j.status === 'In Progress' ? 'bg-green-500/20 text-green-400' :
                        j.status === 'Scheduled' ? 'bg-blue-500/20 text-blue-400' : 'bg-gray-200 text-gray-500'
                      }`}>{j.status}</span>
                    </div>
                  ))}
                </div>
              ...
              : (
                <div className="space-y-2">
                  {todayCrewNames.map((name: any) => {
                    ...
                    return (
                      <div key={String(name)} className="flex items-start gap-2 bg-gray-50/60 rounded-lg px-3 py-2">
                        <div className="flex-1 min-w-0">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        {/* Revenue chart */}
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          <p className="text-sm font-semibold text-gray-900 mb-4">Revenue — last 8 months</p>
          ...
          <div className="flex gap-4 mt-2">
            <div className="flex items-center gap-1.5 text-xs text-gray-500"><div className="w-2.5 h-2.5 rounded-sm bg-blue-500" />Invoiced</div>
            <div className="flex items-center gap-1.5 text-xs text-gray-500"><div className="w-2.5 h-2.5 rounded-sm bg-green-500" />Received</div>
          </div>
        </div>

        {/* In-progress jobs */}
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          <p className="text-sm font-semibold text-gray-900 mb-4">In-progress cost tracker</p>
              ...
              return (
                <div key={j.id}>
                  <div className="flex justify-between items-baseline mb-1">
                    <span className="text-sm font-medium text-gray-900 truncate">{j.client} <span className="text-gray-500 font-mono text-xs">{j.id}</span></span>
      ...
      {/* Recent activity */}
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        <p className="text-sm font-semibold text-gray-900 mb-4">Quotes sent — awaiting response</p>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-200">
                  {jobs.filter((j: any) => j.quote_status === 'Sent').slice(0, 8).map((j: any) => (
                    <tr key={j.id}>
                      <td className="py-2 text-gray-900">{j.client}</td>
                      <td className="py-2 text-gray-500 font-mono text-xs">{j.id}</td>
                      <td className="py-2 text-gray-900 tabular-nums">{fmtCurrency(j.quote_ex_gst)}</td>
                      <td className="py-2 text-gray-500">{fmtDate(j.quote_sent)}</td>
