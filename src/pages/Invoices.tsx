      ...
      <div className="px-6 py-4 border-b border-gray-200 space-y-3 shrink-0">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <div className="bg-gray-50 rounded-lg p-3">
            ...
            <p className={`text-lg font-bold ${overdue > 0 ? 'text-red-400' : 'text-gray-500'}`}>{overdue}</p>
          </div>
        </div>
        ...
        <div className="flex gap-1.5">
          {STATUS_FILTERS.map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={`text-xs px-3 py-1 rounded-full transition-colors ${statusFilter === s ? 'bg-blue-600 text-gray-900 font-semibold' : 'bg-gray-50 text-gray-500 hover:text-gray-900'}`}>
              {s}
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="divide-y divide-gray-200">
          {filtered.map(inv => {
            ...
            return (
              <div key={inv.id} className="flex items-center px-6 py-4 hover:bg-gray-50/30 transition-colors">
                <button className="flex-1 text-left min-w-0" onClick={() => openEdit(inv)}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="text-sm font-semibold text-gray-900">{inv.client || '—'}</div>
                      <div className="text-xs text-gray-500">{inv.job_id} · {fmtDate(inv.date)}</div>
                    </div>
                    <div className="text-right shrink-0">
                      <div className="text-sm font-bold text-gray-900">{fmtCurrency(inv.total_inc_gst)}</div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} size="lg"
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2 flex items-center gap-2">
            ...
            <label htmlFor="manual-paid" className="text-sm text-gray-600">Mark as paid (override)</label>
          </div>
        ...
        <div className="flex justify-between mt-5 pt-4 border-t border-gray-200">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleSave} disabled={saving}
              className="flex items-center gap-1.5 text-sm px-5 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-gray-900 font-semibold disabled:opacity-50">
